  ...
  return (
    <div className="relative flex flex-row items-center rounded-xl border border-white/[0.08] bg-card p-5 transition-all duration-200 card-glow h-full">
      ...
      <div className="relative ml-auto shrink-0 pt-2 w-[116px] pb-6">
        ...
        <span className="absolute bottom-0 left-0 rounded-full bg-green-500/20 px-1.5 py-0.5 text-xs font-bold text-green-400">{greenDays}</span>
        {breakEvenDays > 0 && (
          <span className="absolute bottom-0 left-1/2 -translate-x-1/2 rounded-full bg-blue-500/20 px-1.5 py-0.5 text-xs font-bold text-blue-400">{breakEvenDays}</span>
        )}
        <span className="absolute bottom-0 right-0 rounded-full bg-red-500/20 px-1.5 py-0.5 text-xs font-bold text-red-400">{redDays}</span>
      </div>
    </div>
  );
}
